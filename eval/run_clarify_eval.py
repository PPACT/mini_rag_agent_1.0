...
async def run_set(rows: list[dict], top: int, sem: asyncio.Semaphore, departments) -> list[tuple[dict, bool, bool]]:
    """并发跑一批题；返回 [(题目, 是否门控通过, 是否判定歧义)]。

    并发是必需的：每题要走完整生产链路（改写 + 精排 + 歧义判定 ≈ 3-4 次 LLM 调用），
    串行跑 117 题需要半小时以上。
    """
    return list(await asyncio.gather(*[_one(r, top, sem, departments) for r in rows]))
...
async def main() -> None:
    ...
    parser.add_argument("--scope", choices=list(SCOPES), default="full",
                        help="可见范围：none(不过滤) / dept(仅本部门) / full(公司+本部门)")
    args = parser.parse_args()
    sem = asyncio.Semaphore(args.concurrency)
    departments = SCOPES[args.scope]
    ...
    print(f"歧义/对抗题 {len(clarify_rows)} 条，清晰题 {len(clear_rows)} 条，"
          f"top={args.top}，范围={args.scope}({departments})\n")

    # 应澄清组
    print("跑「应澄清」组...", flush=True)
    should = await run_set(clarify_rows, args.top, sem, departments)
    # 不应澄清组
    print("跑「不应澄清」组...", flush=True)
    should_not = await run_set(clear_rows, args.top, sem, departments)
